...
class TemperatureAnalysis(hcal_analysis.HcalAnalysis):
	...
	def start(self):
		...
		for run in runs:
			...
			self._histograms.AddPrefix("h_run{}".format(run))
			for rbx in rbxes:
				self._histograms.AddTH2F("sumq_vs_ls_RBX{}".format(rbx), "sumq_vs_ls", "LS", run_lses[run][1]-run_lses[run][0]+1, run_lses[run][0]-0.5, run_lses[run][1]+0.5, "SumQ [fC]", 1000, 0., 1000.)
